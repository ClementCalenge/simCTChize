...
\url{https://doi.org/10.5281/zenodo.XXXXXXX}), and then to place it in
...
assessed this trend using the classical model:
$$
N_t = N_0 \mu^t
$$
...
\subsection{Monitoring of the red deer during five years: no habitat
  selection and random sampling of traps)}
...
Contrary to what was simulated for the roe deer, we did no try to be
as realistic as possible for the ``red deer''. The aim was to generate
movements that cover a large area. Simulations of this process showed
that the Minimum convex polygon of the simulated animals covered a
mean area equal to 469 ha (interquartile range: 383 -- 548 ha), which
is approximately the home range size of the actual red deer
\citep{Kamler2008}. Note that we simulated a constant home-range size
for the five years of the study (i.e., we used the same process for
all years).\\

We then simulated a population decrease that could be considered as
realistic for the red deer. As for the roe deer, we simulated a
population size of 300 red deer on the study area during the first
year (which is a very high density for this species, but this allowed
a comparison with the roe deer). We supposed a sex-ratio of 0.5, an
adult annual survival of 0.76, and a reproduction equal to 0.5 fawns
per female. We simulated this process 1000 times and chose one
...
as a dataset from the package.\\

We then used the function \texttt{simulateCTStudy5years} to simulate
1000 times a five-year camera trap study of this ``red deer''
population, simulating for each year the movement of these ``red
deer'' and their monitoring using 100 camera traps randomly placed on
the study area. Note the use of \texttt{redDeer=TRUE} in this
...
the package. This code is provided for information purposes. Note also
that the resulting list \texttt{redNoHS} is available as a dataset in
the package:
...
\texttt{redNoHS25} is available in the package as a dataset):
...
year 1 and year 5, and the trend $\lambda$ (together with their 90\%
...
\subsection{Remark: On the bias of trend estimation}
...
\providecommand{\natexlab}[1]{#1}
\bibitem[{Calenge et~al.(2024)}]{Calengeinprep}
  Calenge, C. et~al. 2024.
...
\bibitem[{CRPF (2004)}]{CRPF2004}
  ...
  \newblock -- Technical report, Centre r\'{e}gional de la propriété
  forestière de Poitou-Charente.
...
\bibitem[{Rowcliffe et~al.(2008)}]{Rowcliffe2008}
  ...
  \newblock -- Journal of Applied Ecology 45, 1228–1236.
